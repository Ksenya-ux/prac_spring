...
def localize_message(message, locale_name):
    ...
    m = re.fullmatch(
        r"(.+) attacked (.+) with (.+), damage (\d+) hp, (.+) died",
        message,
    )
    if m:
        ...
        return _("{} attacked {} with {}, damage {}, {} died").format(
            ...
            name,
            weapon,
            hp_text(damage, ngettext),
            dead_name,
        )

    m = re.fullmatch(
        r"(.+) attacked (.+) with (.+), damage (\d+) hp, "
        r"(.+) now has (\d+)",
        message,
    )
    if m:
            ...
            if game.field[new_x][new_y] is None:
                ...
                for player_name, player_info in game.players.items():
                    if player_info["pos"] == (new_x, new_y):
                        players_on_cell.append(
                            (player_name, player_info["writer"])
                        )

                if players_on_cell:
                    for player_name, writer in players_on_cell:
                        if name == "jgsbat":
                            monster_message = cowsay.cowsay(
                                message=hello,
                                cowfile=game.jgsbat,
                            )
                        else:
...
def handle_command(username, line):
    ...
    if parts[0] == "moveabs":
        ...
        if x < 0 or x >= FIELD_SIZE or y < 0 or y >= FIELD_SIZE:
            return (
                "Coordinates ({}, {}) are outside field (0-{})".format(
                    x,
                    y,
                    FIELD_SIZE - 1,
                ),
    ...
    if parts[0] == "addmon":
        ...
        while i < len(parts):
            if parts[i] == "hello" and i + 1 < len(parts):
                ...
                if hp <= 0:
                    return "Hitpoints must be positive", False
    ...
    if parts[0] == "movemonsters":
        ...
        return "Moving monsters: {}".format(parts[1]), False
...
async def handle_client(reader, writer):
    ...
    try:
        ...
        while True:
            ...
            response, is_broadcast = handle_command(
                username,
                data.decode().strip(),
            )
            if not response:
...
def serve():
    """
    Синхронная обёртка для запуска сервера из multiprocessing.Process.
    """
